...
$(function () {
	...
	if ($(".js-phone-mask").length) {
		$(".js-phone-mask").mask("+38 (000) 00 00 00 0");
	}
	...
	$("#callback").validate({
		rules: {
			callbackName: "required",
			callbackTel: "required",
		},
		messages: {
			callbackName: "*Обязательное поле",
			callbackTel: "*Обязательное поле",
		},
		submitHandler: function (form) {
	...
	$("#subscribe").validate({
		...
		messages: {
			subscribeEmail: "*Обязательное поле",
		},
		submitHandler: function (form) {
			$(".js-thank").click();
		},

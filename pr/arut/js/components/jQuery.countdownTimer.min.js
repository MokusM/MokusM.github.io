...
(function ($) {
	...
	var methods = {
		...
		destroy: function () {
			this.data(
				"countdowntimer",
				$.extend(true, {}, $.fn.countdowntimer.defaults, {
					destroy: true,
				})
			);
		},
	...
	function givenDate($this, opts, type) {
		...
		if (endDate - startDate > 0) {
			if (type === "withStart" && startDate > new Date()) {
				...
				var setSecondsInterval =
					type == "withnoStart"
						? window["startTime" + id].setSeconds(window["startTime" + id].getSeconds() + opts.tickInterval)
						: window["startDate" + id].setSeconds(window["startDate" + id].getSeconds() + opts.tickInterval);
			}
	...
	function prepareTime($this, opts, years, months, days, hours, minutes, seconds) {
		...
		var s =
			Math.round(years * 31536000 * 100) / 100 +
			Math.round(months * 2628000 * 100) / 100 +
			Math.round(days * 86400 * 100) / 100 +
			Math.round(hours * 3600 * 100) / 100 +
		...
		var minutesFormat =
			format[4] === "!" ? Math.round(Math.floor(s / 60 - (hoursFormat * 3600) / 60 - (daysFormat * 86400) / 60 - (monthsFormat * 2628000) / 60 - (yearsFormat * 31536000) / 60)) : 0;
		...
		var time =
			yearsFormat + opts.timeSeparator + monthsFormat + opts.timeSeparator + daysFormat + opts.timeSeparator + hoursFormat + opts.timeSeparator + minutesFormat + opts.timeSeparator + secondsFormat;
		return time;
	}
	...
	function html($this, time, opts) {
		...
		if (opts.labelsFormat === true && typeof window["regexpMatchFormat_" + $this.attr("id")] === "undefined" && typeof window["regexpReplaceWith_" + $this.attr("id")] === "undefined") {
			...
			for (var a = time.length; a > 0; a--) {
				...
				labelTime +=
					"<span class='displaySection'><span class='numberDisplay'>" + time[itr] + "</span><span class='periodDisplay'>" + labelarr[itr] + "</span><span class='periodDevide'>:</span></span>";
			}
			time = labelTime += "</span>";
	...
	$.fn.countdowntimer.regionalOptions = {
		...
		labels: ["Years", "часов", "часов", "часов", "минут", "Seconds"],
		isRTL: false,
	};
})(jQuery);
